...
def run_simulation(window, seed, anchor_date, d_max_hours):
    ...
    tracker = CooldownTracker(state_dir=COOLDOWN_DIR, run_id=f"agnews_pareto_{run_id}")
    tracker.reset()
    ...
    with mlflow.start_run(run_name=run_id):
        ...
        for day in range(SIM_DAYS):
            ...
            drift_detected, drift_score = run_mmd_drift_check(
                ...
                null_mean, null_std, rng, AGNEWS_INJECTION_DAYS,
            )
            ...
            carbon_immediate = result["carbon_intensity_at_t0"]     * AGNEWS_ENERGY_KWH
            carbon_scheduled = result["carbon_intensity_at_t_star"] * AGNEWS_ENERGY_KWH
...
with DAG(
    dag_id="ag_news_pareto",
    start_date=datetime(2026, 1, 1),
    schedule=None,
    catchup=False,
    tags=["greenmlops", "pareto", "ag_news"],
) as dag:
