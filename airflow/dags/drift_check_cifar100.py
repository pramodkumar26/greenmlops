...
EMBEDDINGS_DIR = Path("/usr/local/airflow/include/data/embeddings/cifar100")
...
def check_drift(**context):
    ...
    def load_day_embeddings(day, force_clean=False):
        if not force_clean and day in CIFAR100_INJECTION_DAYS:
...
with DAG(
    dag_id="drift_check_cifar100",
    start_date=datetime(2026, 1, 1),
    schedule=None,
    catchup=False,
    tags=["greenmlops", "drift", "cifar100"],
) as dag:
