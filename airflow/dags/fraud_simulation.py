...
# Empirical measurement from Week 3 baseline training -- MLflow run: xgboost_fraud_baseline
FRAUD_ENERGY_KWH = 0.001007

FRAUD_PROCESSED = "/usr/local/airflow/include/data/processed/fraud"
...
EXPERIMENT      = "fraud_simulation"

SAMPLES_PER_DAY = 3322
REFERENCE_DAYS  = 7
ROLLING_DAYS    = 3
...
def run_simulation(approach, scheduling_fn, use_drift_check, window, seed, anchor_date):
    ...
    train_df     = pd.read_csv(os.path.join(FRAUD_PROCESSED, "train.csv"))
    ...
    with mlflow.start_run(run_name=run_id):
        ...
        for day in range(SIM_DAYS):
            ...
            carbon_immediate = sched["carbon_at_t0"]     * FRAUD_ENERGY_KWH
            carbon_scheduled = sched["carbon_at_t_star"] * FRAUD_ENERGY_KWH
...
with DAG(
    dag_id="fraud_simulation",
    start_date=datetime(2026, 1, 1),
    schedule=None,
    catchup=False,
    tags=["greenmlops", "simulation", "fraud"],
) as dag:
